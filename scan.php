...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const RENDER_APP_BASE_URL = 'https://noshguard.onrender.com'; // YOUR RENDER URL

    const OCR_API_URL = `${RENDER_APP_BASE_URL}/ocr`;
    const ANALYZE_API_URL = `${RENDER_APP_BASE_URL}/analyze`;
    ...
    function displayError(message = 'An error occurred. Please try again.') {
        ...
        resultArea.style.display = 'none';
        showInitialView(); // Go back to initial choice
    }
    ...
    captureBtn.addEventListener('click', () => {
        if (!currentStream || video.readyState < video.HAVE_METADATA) { // Check if video has metadata (dimensions)
        ...
        const imageDataUrl = canvas.toDataURL('image/jpeg', 0.9); // Use JPEG for smaller size, quality 0.9
    ...
    imageUploadInput.addEventListener('change', (event) => {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                processImage(e.target.result); // e.target.result is the base64 data URL
            }
            reader.onerror = () => {
    ...
    try {
        ...
        aiAdviceEl.textContent = "Waiting for analysis...";
        resultArea.style.display = 'none';

        const ocrData = await fetchOCR(base64ImageData);

        if (!ocrData || !ocrData.ocr_text) {
            throw new Error(ocrData?.error || "OCR failed to return any text.");
        }

        const extractedOcrText = ocrData.ocr_text;
        document.getElementById('form_ocr_text').value = extractedOcrText;
        ocrTextEl.textContent = extractedOcrText;

        showLoading('Analyzing health advice...');
        ...
        displayError("Failed: " + error.message);
    }
}
    ...
    // Async function to fetch OCR results from the backend
    async function fetchOCR(base64Image) {
        const blob = dataURLtoBlob(base64Image);
        if (!blob) { throw new Error("Failed to convert image data for OCR."); }

        const formData = new FormData();
        formData.append('image', blob, 'scan_image.jpg'); // Append blob with a filename

        // Fetch request to the OCR API URL (points to Render backend)
        ...
        const responseData = await response.json(); // Always attempt to parse response as JSON
        if (!response.ok) { // Check if HTTP status code is 200-299
            // If not OK, throw an error using the error message from backend if available
            throw new Error(responseData.error || `OCR API Error (${response.status})`);
        }
        // Your Python backend (user_flask_app_latest_version) returns HTTP 400 if no text is found.
        // This means 'response.ok' will be false, and an error will be thrown here.
        // The 'processImage' function's catch block will then handle this error.
        return responseData; 
    }

    // Async function to fetch AI analysis from the backend
    async function fetchAI(textForAnalysis) {
        // Fetch request to the Analyze API URL (points to Render backend)
        ...
        // Your Python backend (user_flask_app_latest_version) for /analyze returns HTTP 400
        // if ocr_text is considered "missing" by its logic.
        return responseData; 
    }

    // Function to handle form submission for user decision
    function submitUserDecision(decisionChoice) {
        // Ensure OCR and AI processing has completed before allowing submission
        ...
        scanResultForm.submit(); // Submits the form to scan-save.php
    }

    consumeBtn.addEventListener('click', () => submitUserDecision('Yes'));
    avoidBtn.addEventListener('click', () => submitUserDecision('No'));

    // Clean up camera stream when the user leaves the page
    window.addEventListener('beforeunload', stopCameraStream); 

    // Initial UI setup
    showInitialView();
    if (typeof lucide !== 'undefined') { // Check if lucide icon library is available
